...
import { Button, Input, Upload, Alert, Spin } from "antd";
import { useEffect, useState, memo } from "react";
import { Space, Table, message } from "antd";
import { CustomCard } from "../components/styledComponents";
import { getUserAudioCollections, handleFileUpload, deleteAudioTrack } from "../dataHandles/audioService";
...
export const AudioManagement = (props) => {
    ...
    const prepareDataAsync = (eachAudio) => {
        return new Promise((resolve, reject) => {
            // console.time("b64toBlob_time_" + eachAudio?.fileName);
            const fileContent = b64toBlob(eachAudio.fileContent);
            // console.timeEnd("b64toBlob_time_" + eachAudio?.fileName);
            ...
            .then((response) => {
                ...
                Promise.allSettled(promiseList).then((result) => {
                    ...
                    const collection = resolvedResults.map(({ status, value }) => {
                        // console.time("Track_Processing_Time_" + value?.fileName);
                        ...
                        // console.timeEnd("Track_Processing_Time_" + value?.fileName);
